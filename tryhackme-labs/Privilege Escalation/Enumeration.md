...
O objetivo deste laboratório é compreender como vulnerabilidades no kernel do Linux podem ser exploradas para obter privilégios elevados (root), saíndo do User -> Super Admin.

A escalação de privilégios é crucial porque permite alcançar níveis de acesso de administrador de sistema, o que possibilita a execução de ações como:

Redefinir senhas
Contornar os controles de acesso para comprometer dados protegidos
Editar configurações de software
Habilitando a persistência
Alterar os privilégios de usuários existentes (ou novos)
...
A enumeração é o primeiro passo a ser dado após obter acesso a qualquer sistema.

comando retornará o nome do host da máquina de destino
```
hostname
```
Irá imprimir informações do sistema, fornecendo detalhes adicionais sobre o kernel utilizado.
```
uname -a
```
Poderá obter informações sobre a versão do kernel e dados adicionais, como se um compilador (por exemplo, GCC) está instalado.
```
cat /proc/version
```
Este arquivo geralmente contém algumas informações sobre o sistema operacional, mas pode ser facilmente personalizado ou alterado.
```
cat /etc/issue
```
comando encontrar
...
find /home -name flag1.txtLocalize os arquivos com o nome “flag1.txt” no diretório /home.
